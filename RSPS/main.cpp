    ...
    while (window.isOpen())
    {
        ...
        if (rocks.size() > 0 && scissorss.size() > 0 && papers.size() > 0) { //셋 다 1개라도 있을 때: 버튼이 생기면 버튼의 조건까지 추가
            ...
            for (int i = 0; i < rocks.size(); i++) { //rock의 경우
                float rockChaseMagnitude = sqrt(rocks[i].nearest(scissorss).x * rocks[i].nearest(scissorss).x + rocks[i].nearest(scissorss).y * rocks[i].nearest(scissorss).y) * 5;
                //가장 가까운 가위와의 거리: 쫓아가야 함
                float rockAvoidMagnitude = sqrt(rocks[i].nearest(papers).x * rocks[i].nearest(papers).x + rocks[i].nearest(papers).y * rocks[i].nearest(papers).y) * 5;
            ...
            for (int i = 0; i < scissorss.size(); i++) {
                float scissorsChaseMagnitude = sqrt(scissorss[i].nearest(papers).x * scissorss[i].nearest(papers).x + scissorss[i].nearest(papers).y * scissorss[i].nearest(papers).y) * 5;
                //가장 가까운 보자기와의 거리: 쫓아야 함
                float scissorsAvoidMagnitude = sqrt(scissorss[i].nearest(rocks).x * scissorss[i].nearest(rocks).x + scissorss[i].nearest(rocks).y * scissorss[i].nearest(rocks).y) * 5;
                //가장 가까운 바위와의 거리
            ...
            for (int i = 0; i < papers.size(); i++) {
                float paperChaseMagnitude = sqrt(papers[i].nearest(rocks).x * papers[i].nearest(rocks).x + papers[i].nearest(rocks).y * papers[i].nearest(rocks).y) * 5;
                //가장 가까운 바위와의 거리
                float paperAvoidMagnitude = sqrt(papers[i].nearest(scissorss).x * papers[i].nearest(scissorss).x + papers[i].nearest(scissorss).y * papers[i].nearest(scissorss).y) * 5;
                //가장 가까운 가위와의 거리
